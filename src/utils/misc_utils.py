...
def load_scan(path):
    slices = [pydicom.read_file(os.path.join(path,s)) for s in os.listdir(path) if s.endswith('.dcm')]
...
def centroids_calc(regional_max):
    labeled_comp = measure.label(regional_max, connectivity=3) # Connected components
